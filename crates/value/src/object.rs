...
#[repr(transparent)]
pub struct TypeEnvironmentId(pub u32);
...
pub struct BuiltInHooks {
    ...
    pub drop_in_place: unsafe fn(NonNull<()>),
    /// Releases values held by the inline state.
    pub enqueue_children: Option<unsafe fn(NonNull<()>, &DropQueue, TeardownMode)>,
    /// Visits collectable boxes held by the inline state.
    pub visit_children: Option<BuiltInVisitChildren>,
}
...
pub unsafe fn drop_built_in_state<T>(data: NonNull<()>) {
...
impl BuiltInState {
    #[must_use]
    pub(crate) const fn new(hooks: &'static BuiltInHooks, next: Option<NonNull<Self>>) -> Self {
        Self { hooks, next }
    }

    #[must_use]
    pub(crate) fn data_offset(hooks: &BuiltInHooks) -> usize {
    ...
    #[must_use]
    pub(crate) const fn hooks(&self) -> &'static BuiltInHooks {
        self.hooks
    }

    #[must_use]
    pub(crate) const fn next(&self) -> Option<NonNull<Self>> {
        self.next
    }

    pub(crate) const fn set_next(&mut self, next: NonNull<Self>) {
        self.next = Some(next);
    }
}
...
impl InstanceObject {
    ...
    #[must_use]
    pub fn new(heap: &Heap, class: ClassId, slot_count: usize) -> ManagedRef<Self> {
    ...
    #[must_use]
    #[inline(always)]
    pub fn new_initialized_typed_with_layout(
        heap: &Heap,
        class: ClassId,
        slot_count: usize,
        type_environment: TypeEnvironmentId,
    ...
    #[must_use]
    pub fn new_finalizable_typed_with_layout(
        heap: &Heap,
        class: ClassId,
        slot_count: usize,
        type_environment: TypeEnvironmentId,
    ...
    #[must_use]
    #[inline(always)]
    pub fn new_initialized_finalizable_typed_with_layout(
        heap: &Heap,
        class: ClassId,
        slot_count: usize,
        type_environment: TypeEnvironmentId,
    ...
    /// # Errors
    ///
    /// Returns the error from `initialize` if a built-in state fails to initialize.
    pub fn with_built_in_states<E>(
        heap: &Heap,
        class: ClassId,
        slot_count: usize,
        hooks: &[&'static BuiltInHooks],
    ...
    ///
    /// Returns the error from `initialize` if a built-in state fails to initialize.
    pub fn with_built_in_states_typed<E>(
        heap: &Heap,
        class: ClassId,
        slot_count: usize,
        type_environment: TypeEnvironmentId,
    ...
    ///
    /// Returns the error from `initialize` if a built-in state fails to initialize.
    pub fn with_finalizable_built_in_states_typed<E>(
        heap: &Heap,
        class: ClassId,
        slot_count: usize,
        type_environment: TypeEnvironmentId,
    ...
    #[must_use]
    #[inline(never)]
    pub fn read_slot(&self, index: usize) -> Value {
        let slot_count = self.slot_count();
        if index >= slot_count {
            slot_out_of_range(index, slot_count);
        }
    ...
    #[must_use]
    #[inline(always)]
    pub unsafe fn read_slot_unchecked(&self, index: usize) -> Value {
        // SAFETY: the tag and managed handle prove the payload type and lifetime.
    ...
    #[must_use]
    #[inline(always)]
    pub unsafe fn read_int_slot_unchecked(&self, index: usize) -> Option<i64> {
        // SAFETY: the tag and managed handle prove the payload type and lifetime.
    ...
    #[inline(always)]
    pub unsafe fn write_int_slot_unchecked(&self, index: usize, value: i64) {
        // SAFETY: the tag and managed handle prove the payload type and lifetime.
    ...
    #[inline(never)]
    pub fn write_slot(&self, index: usize, value: Value) -> Value {
        let slot_count = self.slot_count();
        if index >= slot_count {
            slot_out_of_range(index, slot_count);
        }
    ...
    ///
    /// `index` must be smaller than this instance's slot count.
    pub unsafe fn write_slot_unchecked_with_unique_receiver(
        &self,
        index: usize,
        value: Value,
    ...
    pub unsafe fn write_fresh_slot_unchecked(&self, index: usize, value: Value) {
        // SAFETY: the tag and managed handle prove the payload type and lifetime.
        let slot = unsafe { self.slots().add(index).as_ref() };
        // SAFETY: the tag and managed handle prove the payload type and lifetime.
    ...
    pub const fn cycle_possible(&self) -> bool {
    ...
    #[inline(never)]
    pub fn mutate_slot<R>(&self, index: usize, callback: impl FnOnce(&mut Value) -> R) -> R {
        let slot_count = self.slot_count();
        if index >= slot_count {
            slot_out_of_range(index, slot_count);
        }
    ...
    #[must_use]
    pub const fn has_built_in(&self) -> bool {
        self.built_in.is_some()
    }

    #[must_use]
    pub fn built_in_ref<T: 'static>(&self) -> Option<&T> {
    ...
    #[must_use]
    pub(crate) const fn built_in_state_head(&self) -> Option<NonNull<BuiltInState>> {
        self.built_in
    }
    ...
    /// taken at most once.
    pub const unsafe fn take_slot_unchecked(&self, index: usize) -> Value {
        // SAFETY: the tag and managed handle prove the payload type and lifetime.

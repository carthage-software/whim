...
pub(crate) fn write_may_alias_inputs(kind: InstructionKind) -> bool {
    !matches!(
        kind,
...
pub(crate) fn operands(kind: InstructionKind) -> Option<&'static [Operand]> {
    match kind {
        instruction_kinds!(
            LoadConstant
                | LoadNull
...
pub(crate) fn instruction_bytes(instruction: Instruction) -> [MaybeUninit<u8>; 8] {
    // SAFETY: both types occupy eight bytes, and MaybeUninit preserves padding.
    unsafe { mem::transmute::<Instruction, [MaybeUninit<u8>; 8]>(instruction) }
}

pub(crate) fn register_at(bytes: [MaybeUninit<u8>; 8], offset: usize) -> Register {
    // SAFETY: operand offsets select the two initialized bytes of a register.
    let value = unsafe { [bytes[offset].assume_init(), bytes[offset + 1].assume_init()] };
    Register::new(u16::from_le_bytes(value))
}
...
pub(crate) fn for_each_register(instruction: Instruction, mut visit: impl FnMut(Register)) -> bool {
...
pub(crate) fn for_each_write_register(
    instruction: Instruction,
...
pub(crate) fn for_each_read_register(
    instruction: Instruction,
    mut visit: impl FnMut(Register),
) -> bool {
...
pub(crate) fn replace_read_register(
    ...
    for operand in operands(instruction.kind())? {
        if operand.access == Access::Read && register_at(bytes, operand.offset) == from {
            ...
            bytes[operand.offset].write(encoded[0]);
            bytes[operand.offset + 1].write(encoded[1]);
        }
    }

    // SAFETY: only register fields changed; the tag, other fields, and padding are preserved.
    Some(unsafe { mem::transmute::<[MaybeUninit<u8>; 8], Instruction>(bytes) })
}

pub(crate) fn remap(instruction: Instruction, mapping: &[Register]) -> Instruction {
    ...
    for (position, operand) in operands.iter().enumerate() {
        ...
        bytes[operand.offset].write(remapped[0]);
        bytes[operand.offset + 1].write(remapped[1]);
    }

    // SAFETY: only register fields changed; the tag, other fields, and padding are preserved.
    unsafe { mem::transmute::<[MaybeUninit<u8>; 8], Instruction>(bytes) }
}

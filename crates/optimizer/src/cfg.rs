...
pub(crate) fn has_shared_switch_table(chunk: &Chunk) -> bool {
...
/// this instruction.
pub(crate) fn branches_or_terminates(instruction: Instruction) -> bool {
    matches!(
        instruction,
        Instruction::Jump { .. }
...
impl Dominators {
    ...
    pub(crate) fn is_reachable(&self, instruction: usize) -> bool {
        self.immediate[self.block_of[instruction]].is_some()
    }

    pub(crate) fn dominates(&self, candidate: usize, target: usize) -> bool {
...
pub(crate) fn is_block_boundary(instruction: Instruction) -> bool {
    matches!(
        instruction,
        Instruction::Jump { .. }
            | Instruction::NumericRegionJump { .. }

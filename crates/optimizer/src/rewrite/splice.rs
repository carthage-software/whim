...
pub(crate) fn splice_replace(chunk: &mut Chunk, at: usize, replacement: &[(Instruction, Span)]) {
    splice_replacements(chunk, &[(at, replacement)]);
}

pub(crate) fn splice_replace_many(
    chunk: &mut Chunk,
...
pub(crate) fn can_insert_straight_line_before(chunk: &Chunk, index: usize) -> bool {
...
pub(crate) fn straight_line_insertion_boundaries(chunk: &Chunk) -> Vec<bool> {
...
pub(crate) fn insert_straight_line_before_many(
    chunk: &mut Chunk,
...
pub(crate) fn insert_straight_line_before(
    chunk: &mut Chunk,
    index: usize,
...
mod tests {
    ...
    use crate::rewrite::splice::insert_straight_line_before_many;

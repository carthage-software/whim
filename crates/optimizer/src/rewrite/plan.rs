...
impl RewritePlan {
    pub(crate) fn for_analysis(analysis: &Analysis<'_>) -> Self {
        Self {
            chunks: analysis
                .chunks()
                .iter()
    ...
    pub(crate) fn is_available(&self, analyzed: &AnalyzedChunk<'_>, index: usize) -> bool {
    ...
    pub(crate) fn has_replacements(&self, analyzed: &AnalyzedChunk<'_>) -> bool {
        self.chunks[analyzed.position]
            .replacements
            .iter()
            .any(Option::is_some)
    ...
    pub(crate) fn remove(&mut self, analyzed: &AnalyzedChunk<'_>, index: usize) -> bool {
        let rewrite = &mut self.chunks[analyzed.position];
        rewrite.prepare(analyzed.chunk.code.len());
    ...
    pub(crate) fn apply(self, unit: &mut CompiledUnit) -> RewriteResult {

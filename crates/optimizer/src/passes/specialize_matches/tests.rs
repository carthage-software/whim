...
fn string_chain_keeps_first_duplicate_target() {
    let heap = Heap::new();
    for input in [b"alpha".as_slice(), b"beta", b"missing"] {
        let mut chunk = string_chain(&heap, input, &[b"alpha", b"alpha", b"beta"]);
        ...
        verify(&chunk).expect("the duplicate-target switch verifies");
    }
}
...
fn string_chain_keeps_tests_reached_after_selected_body_changes_subject() {
    let heap = Heap::new();
    for input in [b"alpha".as_slice(), b"beta", b"missing"] {
        let mut chunk = string_chain(&heap, input, &[b"alpha", b"beta"]);
        let changed = chunk
            .add_constant(Literal::String(heap.intern(b"beta")))
            .unwrap();
        ...
        verify(&chunk).expect("the subject-changing switch verifies");
    }
}

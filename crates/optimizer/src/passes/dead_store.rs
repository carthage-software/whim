...
pub(in crate::passes) fn optimize_chunk_with_context(
    chunk: &mut Chunk,
...
pub(in crate::passes) fn optimize_chunk_without_flow(
    chunk: &mut Chunk,
    configuration: OptimizationConfiguration,
    statistics: &mut OptimizationStatistics,
) {
...
pub(in crate::passes) fn scalar_write_is_unobservable(
    chunk: &Chunk,
...
pub(in crate::passes) struct PreviousValueSafety {
    safe_before: Vec<Option<Register>>,
}

impl PreviousValueSafety {
    pub(in crate::passes) fn analyze(
        chunk: &Chunk,
        targets: &HashSet<usize>,
        first_fresh_local: u16,
    ) -> Self {
    ...
    pub(in crate::passes) fn cannot_own_reference(&self, index: usize, register: Register) -> bool {

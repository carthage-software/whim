...
pub(crate) fn normalize_unit(unit: &mut CompiledUnit, configuration: OptimizationConfiguration) {
    for_each_mutable_chunk(unit, configuration, normalize_chunk);
}

pub(crate) fn optimize_chunk(chunk: &mut Chunk, statistics: &mut OptimizationStatistics) {
    if !chunk
        .code
        .iter()
...
pub(crate) fn normalized_chunk(chunk: &Chunk) -> Cow<'_, Chunk> {
    if chunk
        .code
        .iter()
...
pub(crate) fn normalize_chunk(chunk: &mut Chunk) {
    if !chunk
        .code
        .iter()
        .any(|instruction| split(*instruction).is_some())

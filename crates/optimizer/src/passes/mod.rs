...
pub(crate) fn chunk_mut(unit: &mut CompiledUnit, location: FunctionLocation) -> &mut Chunk {
    match location {
...
pub(in crate::passes) fn for_each_mutable_chunk(
...
pub(in crate::passes) fn compact_removed_instructions(
    chunk: &mut Chunk,
    remove: &[bool],
...
pub(in crate::passes) fn plan_type_specializations(
...
pub(in crate::passes) fn specialize_chunk_instructions(
    chunk: &mut Chunk,
    heap: &Heap,
    specialize: for<'flow> fn(&TypeFlow<'flow>, usize, Instruction) -> Option<Instruction>,
) -> usize {
...
pub(crate) fn fuse_lowered_property_updates(
    unit: &mut CompiledUnit,
    configuration: OptimizationConfiguration,
    statistics: &mut OptimizationStatistics,
) {
...
pub(crate) fn optimize_chunk_before_numeric_loop(
    chunk: &mut Chunk,
    allocator: &Heap,
    configuration: OptimizationConfiguration,
    statistics: &mut OptimizationStatistics,
...
pub(crate) fn optimize_initializer_chunk_before_numeric_loop(
    chunk: &mut Chunk,
    allocator: &Heap,
    configuration: OptimizationConfiguration,
    statistics: &mut OptimizationStatistics,
...
pub(crate) fn optimize_isolated_live_tail(
    chunk: &mut Chunk,
    allocator: &Heap,
    configuration: OptimizationConfiguration,
    statistics: &mut OptimizationStatistics,
...
pub(crate) fn optimize_function_before_numeric_loop(
    function: &mut CompiledFunction,
...
pub(crate) fn optimize_numeric_loop(chunk: &mut Chunk, configuration: OptimizationConfiguration) {
    fuse_numeric_loop::optimize_chunk(chunk, configuration);
    hoist_loop_entry::optimize_chunk(chunk, configuration);
}

pub(crate) fn optimize_integer_step_loops(
    chunk: &mut Chunk,
    configuration: OptimizationConfiguration,
    statistics: &mut OptimizationStatistics,
) {
...
pub(crate) fn optimize_unit_numeric_loops(
    unit: &mut CompiledUnit,
    configuration: OptimizationConfiguration,
) {
    for_each_mutable_chunk(unit, configuration, |chunk| {

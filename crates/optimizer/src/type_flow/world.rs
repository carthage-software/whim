...
pub(crate) enum WorldUnitIter<'a> {
...
impl<'a> World<'a> {
    ...
    pub(crate) fn units(&self) -> WorldUnitIter<'_> {
        self.units.iter()
    }
...
impl WorldCache {
    ...
    pub fn push(&mut self, unit: Rc<CompiledUnit>) {
...
impl<'a> IndexedUnit<'a> {
    ...
    pub(crate) fn function_by_name(&self, name: &Atom) -> Option<&CompiledFunction> {
    ...
    pub(crate) fn local_function_index(&self, name: &Atom) -> Option<usize> {
        self.functions_by_name.get(name.as_bytes()).copied()
    }

    pub(crate) fn built_in_function_by_name(
        &self,
        name: &Atom,
    ...
    pub(crate) fn class_by_name(&self, name: &Atom) -> Option<&CompiledClassLike> {
        self.declaration_by_name(
            name,
    ...
    pub(crate) fn constant_by_name(&self, name: &Atom) -> Option<&CompiledConstant> {
    ...
    pub(crate) fn newtype_by_name(&self, name: &Atom) -> Option<&CompiledNewtype> {
        self.declaration_by_name(
            name,
    ...
    pub(crate) fn descriptor_mask(&self, descriptor: &TypeDescriptor, depth: usize) -> Option<u16> {
    ...
    pub(crate) fn expand_aliases(&self, descriptor: &TypeDescriptor) -> TypeDescriptor {

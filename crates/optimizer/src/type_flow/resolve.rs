...
impl<'a> TypeFlow<'a> {
    ...
    pub(in crate::type_flow) fn precise_result(&self, index: usize) -> Option<(Register, Fact)> {
    ...
    pub(crate) fn resolved_function(&self, index: usize) -> Option<&'a CompiledFunction> {
    ...
    pub(crate) fn resolved_built_in_function(
        &self,
        index: usize,
    ...
    pub(crate) fn resolved_property(
        &self,
        index: usize,
        object: Register,
        cache: IcSlot,
    ...
    pub(crate) fn origin_type(&self, origin: u32, depth: usize) -> Option<TypeDescriptor> {
        if depth > MAX_TYPE_DEPTH {
            return None;
        }
    ...
    pub(crate) fn origin_descriptor(&self, origin: u32, depth: usize) -> Option<&TypeDescriptor> {
        if depth > MAX_TYPE_DEPTH {
            return None;
        }
        if origin & PARAMETER_ORIGIN != 0 && origin != THIS_ORIGIN {
    ...
    pub(in crate::type_flow) fn exact_class_specialization(
        &self,
        fact: Fact,
        depth: usize,
    ) -> Option<ExactClass<'a>> {
    ...
    pub(crate) fn class(&self, name: &Atom) -> Option<&'a CompiledClassLike> {
        self.class_like(name)
            .filter(|class| class.kind == ClassLikeKind::Class)
    }

    pub(crate) fn class_like(&self, name: &Atom) -> Option<&'a CompiledClassLike> {
        self.unit?.class_by_name(name)
    }

    pub(crate) fn final_class(&self, name: &Atom) -> Option<&'a CompiledClassLike> {
        self.class(name).filter(|class| class.is_final)
    }

    pub(crate) fn resolved_method_at(
        &self,
        index: usize,
        depth: usize,
    ...
    pub(crate) fn method_return_type_at(
        &self,
        index: usize,
        depth: usize,
    ) -> Option<TypeDescriptor> {
    ...
    pub(crate) fn member_name(&self, cache: IcSlot) -> Option<&'a Atom> {
    ...
    pub(in crate::type_flow) fn equivalent_class_constant_origins(
        &self,
        left: u32,
        right: u32,
    ) -> bool {
    ...
    pub(in crate::type_flow) fn equivalent_final_class_origins(
        &self,
        left: u32,
        right: u32,
    ) -> bool {
    ...
    pub(crate) fn this_arguments_match(&self, arguments: Option<&[TypeDescriptor]>) -> bool {

...
use crate::type_flow::TypeFlowOptions;
use crate::type_flow::World;
use crate::type_flow::descriptor_options_equal;

/// One chunk of the imaged unit, with the facts that hold before each of its
/// instructions.
pub(crate) struct AnalyzedChunk<'a> {
    pub(crate) position: usize,
    pub(crate) location: FunctionLocation,
    ...
    pub(crate) function_name: Option<&'a Atom>,
    /// Whether register zero holds a receiver.
    pub(crate) has_receiver: bool,
    pub(crate) incoming_register_count: u16,
    pub(crate) return_type: Option<&'a TypeDescriptor>,
    pub(crate) candidates: CandidateSet,
    pub(crate) flow: TypeFlow<'a>,
}
...
impl<'a> Analysis<'a> {
    ...
    pub(crate) fn of_early_operations(
        indexed: &'a IndexedUnit<'a>,
        configuration: OptimizationConfiguration,
        heap: &'a Heap,
    ) -> Self {
    ...
    pub(crate) fn chunks(&self) -> &[AnalyzedChunk<'a>] {
        &self.chunks
    }
}

pub(crate) fn annotate_capture_types(unit: &mut CompiledUnit, world: &World<'_>, heap: &Heap) {

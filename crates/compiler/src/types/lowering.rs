...
use crate::types::DeclaredTypeKind;
use crate::types::TypeScope;
use crate::types::rendering::check_arity;
use crate::types::rendering::check_tuple_type;
use crate::types::rendering::check_type_arguments;
use crate::types::rendering::flatten_intersection;
use crate::types::rendering::flatten_union;
use crate::types::rendering::validate_composition;

pub(crate) fn lower_checked_type(
    scope: &TypeScope<'_>,
    source: &Type<'_>,
...
pub(crate) fn lower_pattern_type(
    scope: &TypeScope<'_>,
    source: &Type<'_>,
) -> Result<TypeDescriptor, CompileError> {
    reject_bare_uninhabited_checked_type(source)?;
...
pub(crate) fn lower_type_argument(
    scope: &TypeScope<'_>,
    source: &Type<'_>,
) -> Result<TypeDescriptor, CompileError> {
...
pub(in crate::types) fn is_wildcard(source: &Type<'_>) -> bool {
    matches!(
        source,
        Type::Named(named)
            if matches!(
...
pub(crate) fn reject_return_only_annotation(
    source: &Type<'_>,
    position: &str,
) -> Result<(), CompileError> {
...
pub(crate) fn validate_return_annotation(source: &Type<'_>) -> Result<(), CompileError> {

...
pub(crate) const REGISTER_CAPACITY: usize = u16::MAX as usize;
...
impl Registers {
    ...
    pub(crate) fn reserve_local(&mut self) -> Option<Register> {
    ...
    pub(crate) const fn mark(&self) -> u16 {
        self.next
    }

    pub(crate) const fn temporary_floor(&self) -> u16 {
        self.floor
    }

    pub(crate) fn release_to(&mut self, mark: u16) {
    ...
    pub(crate) const fn release_temporaries(&mut self) {
        self.next = self.floor;
    }

    pub(crate) const fn pin_temporaries(&mut self) -> u16 {
        let saved = self.floor;
        self.floor = self.next;
        saved
    }

    pub(crate) fn unpin_temporaries(&mut self, saved: u16) {
        if saved > self.floor {
    ...
    pub(crate) const fn local_count(&self) -> u16 {
        self.floor
    }
}

...
pub(crate) const COUNT_LIMIT: usize = 64;
pub(crate) const TUPLE_LIMIT: usize = 12;
pub(crate) const WINDOW_CAPACITY: usize = u8::MAX as usize;

/// One extra slot lets a method window hold the receiver and all arguments.
const _: () = assert!(COUNT_LIMIT < WINDOW_CAPACITY);

pub(crate) fn check_sequence<'item, T>(
...
pub(crate) fn check_tuple_sequence<'item, T>(
    kind: CompileErrorKind,
    subject: &str,
    item: &str,
    items: impl IntoIterator<Item = &'item T>,
...
pub(crate) fn check_tuple_count(
    kind: CompileErrorKind,
    subject: &str,
    item: &str,
    count: usize,

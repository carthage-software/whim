...
#[derive(Default)]
pub(in crate::emit) struct ControlFlow<'arena> {
    pub(in crate::emit) frames: Vec<ControlFrame<'arena>>,
}

/// One enclosing control-flow construct.
pub(in crate::emit) enum ControlFrame<'arena> {
    Loop(LoopFrame),
...
#[derive(Clone)]
pub(in crate::emit) enum Cleanup<'arena> {
    Finally(Block<'arena>),
    Using(usize),
}

/// Splits a protected range around the holes punched into it.
pub(in crate::emit) fn subtract_holes(
    start: u32,
    end: u32,
    holes: &[(u32, u32)],
...
pub(in crate::emit) struct LoopFrame {
    /// The `break` jumps, patched to just after the loop.
    pub(in crate::emit) break_jumps: Vec<u32>,
    /// The `continue` jumps, patched to the loop's continuation point.
    pub(in crate::emit) continue_jumps: Vec<u32>,
    /// The assignment state at each abrupt exit.
    pub(in crate::emit) escape_states: Vec<Vec<bool>>,
}

impl LoopFrame {
    pub(in crate::emit) const fn new() -> Self {
        Self {
            break_jumps: Vec::new(),
...
#[derive(Clone, Copy)]
pub(in crate::emit) enum LoopJump {
    Break,
    Continue,
}

/// Pops the finally frame a `try` pushed, returning its punched holes.
pub(in crate::emit) fn pop_finally_holes(
...
pub(in crate::emit) fn pop_loop_frame(flow: &mut ControlFlow<'_>) -> LoopFrame {
    match flow.frames.pop() {
...
pub(in crate::emit) fn scan_statements(statements: &[Statement<'_>]) -> Result<(), CompileError> {

...
use crate::emit::AssignmentOperator;
...
use crate::emit::ShortCircuit;
use crate::emit::Span;
use crate::emit::check_tuple_sequence;
use crate::emit::compound_instruction;
use crate::emit::short_circuit_jump;
...
pub(in crate::emit) enum WriteTarget {
    Output,
    OutputLine,
    Error,
    ErrorLine,
...
pub(in crate::emit) enum Place<'arena> {
    Local {
        name: String,
    },
    Property {
...
pub(in crate::emit) struct DictDestructurePlace<'arena> {
    key: Register,
    place: Place<'arena>,
}

/// One fixed position in a prepared destructuring pattern.
pub(in crate::emit) enum DestructurePlace<'arena> {
...
pub(in crate::emit) enum RestPlace<'arena> {
    Bound(Box<Place<'arena>>),
    Discarded,
}

#[derive(Clone, Copy)]
pub(in crate::emit) enum ChainStep {
    Index(Register),
...
impl BodyCompiler<'_, '_> {
    ...
    pub(crate) fn assignment_discarded(
        &mut self,
        scope: &Scope<'_>,
        assignment: &Assignment<'_>,
    ) -> Result<(), CompileError> {
    ...
    pub(in crate::emit) fn prepare_place<'arena>(
        &mut self,
        scope: &Scope<'_>,
    ...
    pub(in crate::emit) fn prepare_chain<'arena>(
        &mut self,
        scope: &Scope<'_>,
    ...
    pub(in crate::emit) fn materialize_place(
        &mut self,
        place: &mut Place<'_>,
        span: Span,
    ) -> Result<(), CompileError> {
    ...
    /// has `n` levels.
    pub(in crate::emit) fn materialize_levels(
        &mut self,
    ...
    pub(in crate::emit) fn read_place(
        &mut self,
        place: &Place<'_>,
        span: Span,
    ) -> Result<Register, CompileError> {
    ...
    pub(in crate::emit) fn write_place(
        &mut self,
        scope: &Scope<'_>,
        place: &Place<'_>,
        value: Register,
    ...
    pub(crate) fn static_property_cache(
        &mut self,
        scope: &Scope<'_>,

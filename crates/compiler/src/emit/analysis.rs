...
use crate::emit::AssignmentTarget;
use crate::emit::DestructureTarget;
use crate::emit::Expression;
use crate::emit::ParameterList;
use crate::emit::Span;
...
pub(in crate::emit) fn collect_assigned_in_expression(expression: &Expression<'_>) -> Vec<String> {
...
pub(crate) fn collect_assigned_in_statements(statements: &[Statement<'_>]) -> Vec<String> {
    let mut names = Names::default();
...
pub(in crate::emit) fn collect_scoped_bindings_in_statements(
...
pub(crate) fn collect_scoped_bindings_in_statement(
...
pub(in crate::emit) fn collect_scoped_bindings_in_expression(
...
pub(crate) fn closure_has_captures(closure: &Closure<'_>) -> bool {
    let mut names = Names::default();
    collect_closure_free_variables(closure, &mut names);
    !names.ordered.is_empty()
}

pub(in crate::emit) fn collect_free_variables_in_expression(
    expression: &Expression<'_>,
) -> Vec<String> {
    let mut names = Names::default();
    let mut locals = Names::default();
...
pub(in crate::emit) fn collect_free_variables_in_statements(
    statements: &[Statement<'_>],
) -> Vec<String> {
    let mut names = Names::default();
    let mut locals = Names::default();
...
pub(in crate::emit) fn collect_variables_in_statements(
    statements: &[Statement<'_>],
) -> Vec<String> {
    let mut names = Names::default();
    for statement in statements {
...
pub(in crate::emit) fn collect_variables_in_expression(expression: &Expression<'_>) -> Vec<String> {
    let mut names = Names::default();
    collect_variables(Node::Expression(expression), &mut names);
    names.into_owned()
}

pub(crate) fn collect_variables_in_statement(statement: &Statement<'_>) -> Vec<String> {
    let mut names = Names::default();
    collect_variables(Node::Statement(statement), &mut names);
    names.into_owned()
}

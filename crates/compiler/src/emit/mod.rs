...
use whim_bytecode::chunk::SideTableFull;
...
use crate::limits::check_tuple_count;
use crate::limits::check_tuple_sequence;
use crate::names::Resolver;
use crate::registers::REGISTER_CAPACITY;
use crate::registers::Registers;
use crate::types::ClassContext;
use crate::types::GenericTable;
use crate::types::TypeScope;
use crate::types::lowering::lower_checked_type;
use crate::types::lowering::lower_pattern_type;
use crate::types::lowering::lower_type;
use crate::types::rendering::check_call_type_argument_arity;
use crate::types::rendering::check_type_argument_arity;

pub(crate) struct Scope<'compilation> {
    /// The engine heap names are interned in.
    pub(crate) heap: &'compilation Heap,
    /// The path of the source file being compiled.
    pub(crate) runtime_path: &'compilation [u8],
    /// Byte offsets for the start of each source line.
    pub(crate) line_starts: &'compilation [u32],
    ...
    pub(crate) forbidden_binders: Vec<String>,
    ...
    pub(crate) embedded_files: &'compilation EmbeddedFiles,
    pub(crate) target: &'compilation Target,
    ...
    pub(crate) trusted_returns: bool,
}
...
#[derive(Clone, Copy, PartialEq, Eq)]
pub(in crate::emit) enum ValueUse {
    Needed,
    Discarded,
}
...
impl ReturnKind {
    pub(crate) const fn callable(returns_void: bool, returns_never: bool) -> Self {
        if returns_void {
            Self::Void
...
pub(crate) struct BodyCompiler<'compilation, 'arena> {
...
pub(crate) fn integer_gate(value: u64, negated: bool, span: Span) -> Result<i64, CompileError> {
...
use crate::emit::analysis::collect_assigned_in_expression;
use crate::emit::analysis::collect_scoped_bindings_in_expression;
use crate::emit::analysis::collect_scoped_bindings_in_statements;
use crate::emit::analysis::collect_variables_in_expression;
use crate::emit::analysis::collect_variables_in_statements;
use crate::emit::constructs::tuple_index;
use crate::emit::expressions::operators::ShortCircuit;
use crate::emit::expressions::operators::binary_instruction;
use crate::emit::expressions::operators::compound_instruction;
use crate::emit::expressions::operators::short_circuit_jump;
use crate::emit::flow::Cleanup;
use crate::emit::flow::ControlFlow;
use crate::emit::flow::ControlFrame;
use crate::emit::flow::LoopFrame;
use crate::emit::flow::LoopJump;
use crate::emit::flow::pop_finally_holes;
use crate::emit::flow::pop_loop_frame;
use crate::emit::flow::scan_statements;
use crate::emit::flow::subtract_holes;
use crate::emit::places::ChainStep;
use crate::emit::places::Place;
use crate::emit::places::WriteTarget;
...
impl<'compilation, 'arena> BodyCompiler<'compilation, 'arena> {
    ...
    pub(crate) fn parameter_prologue(
        &mut self,
        scope: &Scope<'_>,
        parameter_list: &ParameterList<'_>,
    ) -> Result<(), CompileError> {

...
use crate::emit::expressions::Access;
use crate::emit::expressions::BodyCompiler;
use crate::emit::expressions::ChainStep;
use crate::emit::expressions::CompileError;
use crate::emit::expressions::CompileErrorKind;
use crate::emit::expressions::Expression;
use crate::emit::expressions::HasSpan;
use crate::emit::expressions::Place;
...
#[derive(Clone, Copy)]
pub(in crate::emit) enum ShortCircuit {
    And,
    Or,
    Coalesce,
}

pub(in crate::emit) const fn short_circuit_of(operator: BinaryOperator) -> Option<ShortCircuit> {
    match operator {
...
pub(in crate::emit) const fn short_circuit_jump(
    kind: ShortCircuit,
    condition: Register,
) -> Instruction {
    match kind {
...
pub(in crate::emit) fn binary_instruction(
...
pub(in crate::emit) fn compound_instruction(
...
impl BodyCompiler<'_, '_> {
    ...
    pub(in crate::emit::expressions) fn unary_prefix(
        &mut self,
        scope: &Scope<'_>,
        unary: &UnaryPrefix<'_>,
    ) -> Result<Register, CompileError> {
    ...
    pub(in crate::emit::expressions) fn unary_prefix_discarded(
        &mut self,
        scope: &Scope<'_>,
        unary: &UnaryPrefix<'_>,
    ) -> Result<(), CompileError> {
    ...
    pub(in crate::emit::expressions) fn unary_postfix(
        &mut self,
        scope: &Scope<'_>,
    ...
    pub(in crate::emit::expressions) fn unary_postfix_discarded(
        &mut self,
        scope: &Scope<'_>,
        unary: &UnaryPostfix<'_>,
    ) -> Result<(), CompileError> {

...
#[whim_function("Whim\\_Private\\uuid_v4(): string", must_use)]
pub(crate) fn v4(context: &Context<'_, '_, '_>) -> Value {
    context.string(RawUuid::new_v4().as_bytes())
}

#[whim_function("Whim\\_Private\\uuid_v7(): string", must_use)]
pub(crate) fn v7(context: &Context<'_, '_, '_>) -> Value {
    context.string(RawUuid::now_v7().as_bytes())
}

#[whim_function("Whim\\_Private\\uuid_parse(string $value): null|string", must_use)]
...
#[whim_function("Whim\\_Private\\uuid_format(string $bytes): null|string", must_use)]
pub(crate) fn format(context: &Context<'_, '_, '_>, arguments: Arguments<'_>) -> Value {
    let bytes = arguments.bytes(0);
    let Ok(uuid) = RawUuid::from_slice(bytes) else {
        return Value::null();
    };
    let mut buffer = RawUuid::encode_buffer();
...
#[whim_function("Whim\\_Private\\uuid_version(string $bytes): null|1..=8", must_use)]
